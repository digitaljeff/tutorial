      ...
      <header className="flex items-end justify-between">
        <div>
          <div className="text-xs text-zinc-600">
            <Link href={`/shows/${sid}`} className="hover:text-zinc-400">{show.title}</Link> /{" "}
            <Link href={`/shows/${sid}/episodes`} className="hover:text-zinc-400">episodes</Link> / {id}
          </div>
          <h1 className="mt-1 text-2xl font-bold text-zinc-50">{m.script.title}</h1>
        ...
        <a href={videoUrl} download={`${id}.mp4`}
          className="rounded bg-zinc-800 px-3 py-1.5 text-sm text-zinc-300 hover:bg-zinc-700">Download MP4</a>
      </header>
          ...
          {m.shots.map((s: any) => (
            <div key={s.idx}
              ...
              <div className="mt-2 text-[11px] text-zinc-500">
                ...
                {s.qc && s.qc.pass === false && <span className="ml-1 rounded bg-amber-400/10 px-1 text-amber-400">QC</span>}
              </div>
              ...
              <form action={startRetake} className="mt-2 flex gap-1.5">
                ...
                <input type="hidden" name="show" value={sid} />
                ...
                <button className="rounded bg-amber-400 px-2 py-1 text-[11px] font-semibold text-zinc-900 hover:bg-amber-300">Retake</button>
              </form>
            </div>
          ))}
        </div>
